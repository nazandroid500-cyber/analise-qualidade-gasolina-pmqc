...
def plotar_conformidade(resumo):
    ...
    plt.tight_layout()
    plt.savefig('grafico_conformidade_por_uf.png', dpi=150)
...
def boxplot_por_uf(df_ensaio, nome_ensaio):
    ...
    plt.boxplot(dados, labels=ufs, showfliers=True)
...
def mapa_conformidade(resumo):
    ...
    estados = gpd.read_file("brazil-states.geojson")
    ...
    plt.savefig("mapa_conformidade_brasil.png", dpi=150)

    print("Mapa mapa_conformidade_brasil.png gerado")
...
def main(caminho_csv):
    ...
    resumo.to_csv('resumo_conformidade_uf.csv', sep=';')
