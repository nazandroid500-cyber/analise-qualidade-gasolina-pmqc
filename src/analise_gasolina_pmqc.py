...
def plotar_conformidade(resumo):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'grafico_conformidade_por_uf.png'), dpi=150)  
    plt.close() 
...
def plotar_histograma(df_ensaio, nome_ensaio):
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, nome_arquivo), dpi=150)  
    plt.close()
...
def main():  
    caminho_csv = os.path.join(DATA_DIR, 'pmqc_2025_06.csv')  # este "CAMINHO" é FIXO
